<template>
  <div
    ...
    :style="`width: 100%; height: calc(var(--vh, 1vh) * 100); z-index: 4; background: ${
      $vuetify.breakpoint.xsOnly && comboboxFocus ? $vuetify.theme.currentTheme.background : 'unset'
    }`"
  >
    <v-card class="rounded-lg" :style="`width: ${$vuetify.breakpoint.xsOnly ? '100%' : '360px'}`">
      <div v-if="$vuetify.breakpoint.smAndUp && !currentTheme" class="pa-2">
        <v-btn
      ...
      <div class="overflow-x-hidden overflow-y-auto" style="position: relative; max-height: 250px">
...
export default {
  ...
  computed: {
    ...
    globalIndicators() {
      return (
        this.getGroupedFeatures &&
        this.getGroupedFeatures
          .filter((f) => ["global"].includes(f.properties.indicatorObject.siteName))
          .sort((a, b) =>
            a.properties.indicatorObject.indicatorName > b.properties.indicatorObject.indicatorName
              ? 1
              : -1
          )
      );
  ...
  methods: {
    ...
    getSearchItems() {
      const itemArray = [
        ...
        ...this.getFeatures.map((f) => ({
          ...f,
          name: `${f.properties.indicatorObject.city}: ${this.getIndicator(
            f.properties.indicatorObject
          )}`,
        })),
      ];
        ...
        queryParts
          ...
          .forEach((p) => {
            if (p !== queryParts[0] ? !skip.includes(p) : true) {
              const countryName = countries.features.find(
                (c) => c.properties.alpha2 === searchItem.properties?.indicatorObject?.country
              )?.properties.name;
              ...
              if (
                countries.features
                  ...
                  .some((i) => i.toLocaleLowerCase().includes(p)) &&
    ...
    getIndicator(indObj) {
      ...
      if (
        this.baseConfig.indicatorsDefinition[indObj.indicator] &&
  ...
  watch: {
    ...
    selectedIndicator(indicatorObject) {
      ...
      this.selectedMapLayer = this.globalIndicators.findIndex(
        (l) =>
          this.getLocationCode(l.properties.indicatorObject) ===
    ...
    selectedListItem(input) {
      ...
      if (parsedInput.indicator) {
        ...
        this.setSelectedIndicator(parsedInput.properties.indicatorObject);
      } else {
    ...
    selectedMapLayer(index) {
      if (index >= 0) {
        this.setSelectedIndicator(this.globalIndicators[index]?.properties.indicatorObject);
      }
    },
    userInput(newInput) {
      this.sortSearchItems();
