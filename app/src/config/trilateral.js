...
export const layerNameMapping = Object.freeze({
  ...
  'ALOS-2': {
    ...
    tileSize: 256,
    attribution: '{ <a href="https://eodashboard.org/terms_and_conditions" target="_blank">Use of this data is subject to Articles 3 and 8 of the Terms and Conditions</a> }',
    dateFormatFunction: (date) => Datetimes.fromISO(date).toFormat('yyyy_MM_dd'),
  },
  GOSAT_XCO2: {
    ...
    tileSize: 256,
    attribution: '{ <a href="https://eodashboard.org/terms_and_conditions" target="_blank">Use of this data is subject to Articles 3 and 8 of the Terms and Conditions</a> }',
    dateFormatFunction: (date) => Datetimes.fromISO(date).toFormat('yyyyMM'),
    siteMapping: (eoID) => {
      const mapping = {
        CN01: 'be',
        CN02: 'sh',
...
const getMonthlyDates = (start, end) => {
  let currentDate = Datetimes.fromISO(start);
  const stopDate = Datetimes.fromISO(end);
  const dateArray = [];
  while (currentDate <= stopDate) {
    dateArray.push(Datetimes.fromISO(currentDate).toFormat('yyyy-MM-dd'));
    currentDate = Datetimes.fromISO(currentDate).plus({ months: 1 });
  }
  return dateArray;
};
...
const getDailyDates = (start, end) => {
  ...
  while (currentDate <= stopDate) {
    dateArray.push(Datetimes.fromISO(currentDate).toFormat('yyyy-MM-dd'));
    currentDate = Datetimes.fromISO(currentDate).plus({ days: 1 });
  }
  return dateArray;
};
...
const getWeeklyDates = (start, end) => {
  ...
  while (currentDate <= stopDate) {
    dateArray.push(Datetimes.fromISO(currentDate).toFormat('yyyy-MM-dd'));
    currentDate = Datetimes.fromISO(currentDate).plus({ weeks: 1 });
  }
  return dateArray;
};
const wkt = new Wkt();
...
export const globalIndicators = [
  {
    properties: {
      indicatorObject: {
        ...
        indicator: 'N1',
        indicatorValue: ['TROPOMI: Nitrogen dioxide'],
  ...
  },
  {
    properties: {
      indicatorObject: {
        ...
        description: 'Air Quality',
        indicator: 'N1',
        indicatorValue: ['OMI: Nitrogen dioxide'],
        ...
        aoi: null,
        aoiID: 'W2',
        times: getMonthlyDates('2004-10-01', '2020-03-01'),
        inputData: Array(186).fill(['N1']), // just for enabling eo data button for now
        display: {
          protocol: 'xyz',
          maxNativeZoom: 6,
          opacity: 0.7,
  ...
  },
  {
    properties: {
      indicatorObject: {
        ...
        description: 'Greenhouse Gases',
        indicator: 'N2',
        indicatorValue: ['OCO-2: Difference CO2'],
    ...
    properties: {
      indicatorObject: {
        ...
        display: {
          ...
          name: 'Population',
          attribution: '{ <a href="https://eodashboard.org/terms_and_conditions" target="_blank">Use of this data is subject to Articles 3 and 8 of the Terms and Conditions</a> }',
          dateFormatFunction: (date) => Datetimes.fromISO(date).toFormat("yyyy-MM-dd'T'HH:mm:ss'Z'"),
